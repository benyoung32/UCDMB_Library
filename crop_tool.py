...
def savePageImage(page: fitz.Page, filepath: str) -> None:
    pixmap = page.get_pixmap(dpi=300, colorspace="GRAY")  # type: ignore
    pixmap.save(filepath)


def getPagePILImage(parent, page: fitz.Page, dpi=300) -> Image.Image:
    pix = page.get_pixmap(dpi=dpi, colorspace="GRAY")  # type: ignore
    parent.page_image = Image.frombytes("L", 
                    [pix.width, pix.height], pix.samples)  # type: ignore
    # if resize:
    width, height = page.bound().br
    parent.page_image = parent.page_image.resize(size=(int(width), int(height)))
    return parent.page_image  # type: ignore


def createPageScaledImage(p, page: fitz.Page, dpi=300) -> ImageTk.PhotoImage:
    my_image = img = ImageTk.PhotoImage(  # noqa: F841
        getPagePILImage(p, page, dpi))
    return my_image
...
class CropTool(tk.Toplevel):
    def __init__(self, parent, filepath="") -> None:
        ...
        # track multiple keyboard inputs simultaneously
        self.key_history = []
    ...
    def initEntries(self) -> None:
        self.latest_row = 0
        self.preview_button = self.addEntry(
            tk.Button, command=self.createPreviewWindow, text="Preview Result"
        )
        self.print_button = self.addEntry(
            tk.Button,
            command=lambda: print(self.getSettingsString()),
            text="Print Settings",
        )
        self.crop_folder_button = self.addEntry(
            tk.Button, command=self.applyCropToFolder, text="Crop folder"
        )
        self.crop_file_button = self.addEntry(
            tk.Button,
            command=self.applyCropToFile,
            width=20,
            height=1,
            text="Crop file",
        )
        self.open_file_button = self.addEntry(
            tk.Button,
        ...
        self.rotate_entry: EntryWithLabel = self.addEntry(
            EntryWithLabel, label="Rotate?", default=""
        )  # type: ignore
        self.expand_box = self.addEntry(
            CheckButtonWithLabel, label="Expand?", var=self.expand_var
        )
        # self.twoinone_box = self.addEntry(CheckButtonWithLabel,
        #                         label='Two in one?',var=self.twoinone_var)
        self.rightalign_box = self.addEntry(
            CheckButtonWithLabel, label="Right Align?", var=self.rightalign_var
        )

    def addEntry(
        self, entry_type: Callable, column: int = 0, columnspan: int = 1, **args
    ) -> tk.Widget:
        entry = entry_type(self.entry_frame, **args)
        entry.grid(
            row=self.latest_row, column=column, sticky="nwes", columnspan=columnspan
        )
    ...
    def initPDFCanvas(self) -> None:
        ...
        self.myimage = createPageScaledImage(self, page)
    ...
    def updateKeyboardInput(self):
        ...
        if "Left" in key_string:
            target.x = target.x - scale
        if "Up" in key_string:
            target.y = target.y - scale
    ...
    def openFilePrompt(self) -> str:
        # get file info
        answer = tk.filedialog.askopenfile(
            parent=self.master,  # type: ignore
    ...
    def drawCropBox(self) -> None:
        ...
        self.CROPBOX = self.pdf_canvas.create_rectangle(
            self.tl.x, self.tl.y, self.br.x, self.br.y, outline="dodger blue", width=1
        )
    ...
    def getSettingsDict(self) -> dict[str, Any]:
        ...
        args["margins"] = (
            int(tl.y),
            int(tl.x),
            int(self.page_br.x - br.x),
            int(self.page_br.y - br.y),
        )
        args["full_size"] = True
    ...
    def applyCropToFolder(self) -> None:
        ...
        reader.openCropSaveDocs(files, **self.getSettingsDict())

    def applyCropToFile(self) -> None:
        reader.openCropSaveDocs([self.filepath], **self.getSettingsDict())
    ...
    def createPreviewWindow(self) -> None:
        ...
        preview_window = PDFWindow(self.main_frame, newdoc, "Crop Preview")
...
class PDFCanvas(tk.Frame):
    ...
    def __init__(
        self, parent: tk.Widget, doc: fitz.Document, label_var: tk.StringVar = None
    ) -> None:
        tk.Frame.__init__(self, parent)
        self.doc = doc
        ...
        if label_var != "":
            ...
            self.label = tk.Label(self, textvariable=self.label_var, font=MYFONT)
    ...
    def updatePage(self, page_number: int) -> None:
        if not self.page_images[page_number]:
            ...
            self.page_images[page_number] = createPageScaledImage(self, page, dpi=100)
        img = self.page_images[page_number]
        self.pagenum = page_number
        self.updateImage(img)

    def updateImage(self, img: Image.Image) -> None:
        self.img = img
        self.clear()
    ...
    def preloadImages(self) -> None:
        for i, page in enumerate(self.doc):  # type: ignore
            self.page_images[i] = createPageScaledImage(self, page, dpi=100)
